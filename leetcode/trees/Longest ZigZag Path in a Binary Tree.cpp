...
{
public:
    int dfs(TreeNode *root, int isleft, int length)
    {
        if (!root)
            return length;
        ...
        if (isleft == 1)
        {
            return max(dfs(root->right, 0, length + 1), dfs(root->left, 1, 0));
        }
        // similarly, we apply the same logic for the opposite direction
        return max(dfs(root->right, 0, 0), dfs(root->left, 1, length + 1));
    }
